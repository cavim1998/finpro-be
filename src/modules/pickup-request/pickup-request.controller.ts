...
export class PickupRequestController {
  ...
  createPickupRequest = async (req: Request, res: Response) => {
  ...
  getPickupRequests = async (req: Request, res: Response) => {
    const authUser = res.locals.user as { sub?: string };
    if (!authUser?.sub) throw new ApiError("Unauthorized", 401);
    const customerId = parseInt(authUser.sub);
  ...
  getPickupRequestById = async (req: Request, res: Response) => {
    const authUser = res.locals.user as { sub?: string };
    if (!authUser?.sub) throw new ApiError("Unauthorized", 401);
    const customerId = parseInt(authUser.sub);
  ...
  getArrivedPickups = async (req: Request, res: Response) => {
    const authUser = res.locals.user as { sub?: string };
    if (!authUser?.sub) throw new ApiError("Unauthorized", 401);
